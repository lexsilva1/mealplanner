...
import { Container, Grid2 as Grid, Card, CardHeader, CardContent, Typography } from '@mui/material';
...
const Mealplan = ({ mealPlanData }) => {
    ...
    return (
        <Container className="meal-plan-container">
<Grid container spacing={2} className="days-row">
                        ...
                        Object.keys(mealsByDayAndMealOfDay[day.toUpperCase()]).map(
                            (mealOfDay, mealIndex) => (
                                <MealCard
                                    key={`${index}-${mealIndex}`}
                                    mealOfDay={mealOfDay}
                                    mealPlans={mealsByDayAndMealOfDay[day.toUpperCase()][mealOfDay]}
                                />
                            )
                        )
                    ) : (
